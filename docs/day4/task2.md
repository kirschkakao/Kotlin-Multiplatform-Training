...
We have already implemented the behaviour of the light blue cells (_permissible coordinates_) in the `TileGrid`, so this works already.
...
!!! example "Task"
    ...
    - If the current coordinate is a permissible coordinate, we check if the current tile can be placed in that cell by using the `matchesConnections` function of the tile grid in our `GridState`. If the tile can be placed, we render a `TileCell` composable with `tile = null` that has a 2 dp thick border with the `outline` color from our theme.
